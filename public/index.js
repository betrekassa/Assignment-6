...
const fields = ['first_name', 'last_name', 'created_by', 'id'];
...
const parseForm = formId => {
  const user = fields.reduce((user, field) => {
    user[field] = parseField(formId, field); // eg. joke.id = "5"
    return user;
  }, {});
...
const addUser = async e => {
  ...
  const user = parseForm('addForm');
  const mutation = `mutation ($user: userInput!) {
    addUser(user: $user) ${userSelection}
  }`;

  await makeQuery(mutation, { user });
  alert(`Your user name "${user.first_name}" was added successfully!`);
};
